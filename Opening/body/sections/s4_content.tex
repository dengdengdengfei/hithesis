...
\begin{tikzpicture}[
    ...
    block/.style={rectangle, draw, fill=blue!10, text width=4.5cm, text centered, rounded corners, minimum height=1cm, font=\small},
    smallblock/.style={rectangle, draw, fill=green!10, text width=3.5cm, text centered, rounded corners, minimum height=0.8cm, font=\small},
    tinyblock/.style={rectangle, draw, fill=green!5, text width=3.2cm, text centered, rounded corners, minimum height=0.7cm, font=\footnotesize},
    validation/.style={rectangle, draw, fill=orange!10, text width=4cm, text centered, rounded corners, minimum height=0.8cm, font=\small},
    arrow/.style={->, >=stealth, thick}
]

% 顶层框架
\node [block] (framework) {多模态人工智能\\驱动的解析框架};

% 三个核心模块
\node [smallblock, below left=of framework, xshift=-1.5cm] (vision) {视觉语义分割\\（形式表征）};
\node [smallblock, below=of framework] (knowledge) {知识图谱建模\\（规则表征）};
\node [smallblock, below right=of framework, xshift=1.5cm] (reasoning) {跨模态推理\\（语义关联）};

% 具体技术
\node [tinyblock, below=of vision] (vision-detail) {建筑图像 $\rightarrow$\\空间元素识别};
\node [tinyblock, below=of knowledge] (knowledge-detail) {礼制文献 $\rightarrow$\\转译结构形式化};
\node [tinyblock, below=of reasoning] (reasoning-detail) {图谱+LLM $\rightarrow$\\RAG语义问答};

% 案例验证
\node [validation, below=2.5cm of knowledge] (validation) {广府宗祠案例验证};
...
本文基于"理论建构-方法创新-实证验证"的学术路径，整合建筑类型学、符号学、空间句法与人工智能技术，构建适用于规则驱动型建筑的分析框架。
...
系统梳理罗西类型学理论谱系及其方法论遗产，识别其在"形式模糊型"建筑分析中的理论空白。研究文献涵盖：建筑类型学（罗西、迪朗、穆拉托利学派），符号学理论（皮尔斯、艾柯），礼制史料（《朱子家礼》、族谱、方志），以及空间句法、知识图谱等方法论文献。

\noindent\textbf{（2）符号学方法}

基于皮尔斯三元符号理论，采用"编码-解码"分析视角，探究礼制规则如何编码为空间形式，以及空间形式如何被解读为社会意义，揭示"规则-形式转译结构"的运作机制。

\noindent\textbf{（3）空间句法与量化评估}

采用空间句法进行拓扑建模与可达性计算，并引入"身份维度"发展"伦理句法"概念。通过构建基于宗族角色的差异化权限矩阵，量化评估不同身份群体的空间使用许可度，将传统句法的物理可达性分析拓展至社会规则约束下的伦理许可性评估。

\noindent\textbf{（4）知识工程方法}

基于CIDOC CRM设计领域本体，利用大语言模型从文献中抽取实体关系，构建知识图谱。通过图谱推理与RAG架构，实现"规则→形式"和"形式→规则"的双向推演，将隐性礼制知识显性化、可计算化。

\noindent\textbf{（5）实地调查法}

赴广府地区（广州、佛山、中山等地）开展宗祠建筑田野调查，进行测绘、摄影与访谈，获取建筑形制、空间布局及祭祀仪式、族规禁忌等一手资料。

\noindent\textbf{（6）跨案例对比法}

选取不同地域、等级的广府宗祠案例，通过识别"等级→轴线、禁忌→边界、仪式→动线"等转译模式的共性，验证"转译结构持久性"假说，证明类型层面规则-形式转译结构的稳定性。
