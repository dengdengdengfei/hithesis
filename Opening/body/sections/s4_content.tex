...
知识库构建 & 本体建模、知识抽取 & CIDOC CRM本体扩展；基于LLM的自动化知识抽取 \\ \addlinespace
形式分析 & 语义分割、空间句法 & 深度学习模型训练；拓扑计算与伦理句法建模 \\ \addlinespace
规则分析 & 符号学解码、图谱推理 & 知识图谱推理；基于RAG架构的LLM多跳推理 \\ \addlinespace
案例验证 & 田野调查、专家评审 & 实地测绘与影像采集；跨案例对比验证 \\
\bottomrule
\end{tabularx}
\endgroup % 结束局部作用域并自动闭合字体设置
